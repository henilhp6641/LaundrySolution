...
class PrettyDioLogger extends Interceptor {
  ...
  /// Print request data [Options.excelData]
  ...
  Future onRequest(
    ...
    if (requestHeader) {
      _printMapAsTable(options.queryParameters,
          header: 'Query Parameters', color: paramColor);
      ...
      if (data != null) {
        if (data is Map) {
          ...
          _printMapAsTable(formDataMap,
              header: 'Form data | ${data.boundary}', color: paramColor);
        }
  ...
  void _printRequestHeader(RequestOptions options) {
    ...
    _printBoxed(
        header: 'Request ║ $method ', text: uri.toString(), color: infoColor);
  }
...
//      key = '\"$key\"';
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value))
          logPrint(
              vColor!('║${_indent(tabs)} $key: $value${!isLast ? ',' : ''}'));
        else {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          logPrint(
              vColor!('║${_indent(tabs)} $key: $msg${!isLast ? ',' : ''}'));
      }
    });
    ...
    list.asMap().forEach((i, e) {
      ...
      if (e is Map) {
        if (compact && _canFlattenMap(e))
          ...
          _printPrettyMap(e,
              tabs: tabs + 1, isListItem: true, isLast: isLast, vColor: vColor);
      } else
